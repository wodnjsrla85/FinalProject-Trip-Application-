...
class AirplaneEndCreatePage extends StatefulWidget {
  const AirplaneEndCreatePage({super.key});

  @override
  State<AirplaneEndCreatePage> createState() => _AirplaneEndCreatePageState();
}

class _AirplaneEndCreatePageState extends State<AirplaneEndCreatePage> {
  final _formKey = GlobalKey<FormState>();

  // 컨트롤러
  final _flightNo = TextEditingController();    // 운항편명
  final _airline = TextEditingController();     // 항공사
  final _aircraft = TextEditingController();    // 기종
  final _origin = TextEditingController();      // 출발지 (검색)
  final _destination = TextEditingController(text: 'ICN'); // 목적지: 고정 읽기전용
  final _terminal = TextEditingController(text: 'T1');
  final _totalSeats = TextEditingController();

  DateTime? _flightDate;   // 운항일자
  TimeOfDay? _depTime;     // 출발시간
  DateTime? _arrivalDateTime;

  bool _saving = false;
  bool _recomputeScheduled = false;

  List<String> get _originOptions {
  ...
  void _computeArrivalPure() {
    ...
    final ori = _origin.text.trim().toUpperCase();
    final minutes = kFlightTimes[ori];
    if (minutes == null) {
      _arrivalDateTime = null;
      return;
    }
  ...
  bool get _isDirect {
    final ori = _origin.text.trim().toUpperCase();
    return kNonstopFromICN.contains(ori);
  }
  ...
  Future<void> _save() async {
    ...
    final ori = _origin.text.trim().toUpperCase();
    final mins = kFlightTimes[ori];
    if (mins == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('출발지 코드가 유효하지 않습니다.')),
      );
      return;
    }
    if (_flightDate == null || _depTime == null) {
    ...
    try {
      final data = <String, dynamic>{
        ...
        "목적지": _destination.text.trim(), // ICN
        "예상 소요 시간": mins,
        "직항여부": directInt,
        "직항/경유": directTxt,
        "상태": "운행",
      ...
      await FirebaseFirestore.instance.collection('airplane_end').add(data);
      if (!mounted) return;
            ...
            child: Column(
              children: [
                TextFormField(
                  ...
                  decoration: const InputDecoration(labelText: '운항편명 (예: EY8452)'),
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Autocomplete<String>(
                        optionsBuilder: (TextEditingValue textEditingValue) {
                          ...
                          return _originOptions.where((opt) => opt.startsWith(q));
                        },
                        fieldViewBuilder: (context, textController, focusNode, _) {
                          return TextFormField(
                            controller: textController,
                            focusNode: focusNode,
                            decoration: const InputDecoration(labelText: '출발지 (IATA, 예: NRT)'),
                            onChanged: (v) {
                              _origin.text = v.toUpperCase();
                              _scheduleRecompute();
                            },
                            validator: (v) {
                              final code = (v ?? '').trim().toUpperCase();
                              if (code.isEmpty) return '필수 입력';
                              if (!kFlightTimes.containsKey(code)) return '알 수 없는 공항 코드';
                              return null;
                            },
                          );
                        },
                        onSelected: (String selection) {
                          _origin.text = selection.toUpperCase();
                          _scheduleRecompute();
                        },
                      ),
                    ),
                    ...
                    Expanded(
                      child: TextFormField(
                        ...
                        readOnly: true,
                        enabled: false,
                        decoration: const InputDecoration(labelText: '목적지 (ICN)'),
                      ),
                    ),
                  ],
                ),
                ...
                Row(
                  children: [
                    Expanded(child: ListTile(contentPadding: EdgeInsets.zero, title: const Text('예상 소요(hh:mm)'), subtitle: Text(durationHHMM))),
                    const SizedBox(width: 12),
                    Expanded(child: ListTile(contentPadding: EdgeInsets.zero, title: const Text('거리구분'), subtitle: Text(distanceGroup))),
                  ],
                ),
