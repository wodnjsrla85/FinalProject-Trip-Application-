// lib/ui/pages/edit/airplane_end_edit_page.dart
import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:flutter/material.dart';
import '../../../data/airplane_end_repository.dart';
import '../../../utils/flight_meta.dart';

class AirplaneEndEditPage extends StatefulWidget {
  final String docId;
  const AirplaneEndEditPage({super.key, required this.docId});

  @override
  State<AirplaneEndEditPage> createState() => _AirplaneEndEditPageState();
}

class _AirplaneEndEditPageState extends State<AirplaneEndEditPage> {
  final _repo = AirplaneEndRepository();
  ...
  final _origin = TextEditingController();                // 입력/자동완성
  final _destination = TextEditingController(text: 'ICN'); // 고정
  ...
  // 운항 상태 드롭다운 (End도 Start와 동일한 옵션 유지)
  final List<String> _statusOptionsBase = const [
    '운행',
    '운행중단',
    '임시중단',
  ];
  late List<String> _statusOptions; // 문서의 상태가 목록에 없으면 포함하기 위해 가변
  String _status = '운행';
  ...
  Future<void> _load() async {
    try {
      ...
      _origin.text = a.origin;
      _destination.text = a.destination.isEmpty ? 'ICN' : a.destination;
  ...
  Future<void> _save() async {
    ...
    try {
      final data = <String, dynamic>{
        ...
        "목적지": _destination.text.trim(), // ICN (읽기전용이지만 명시 저장)
        "예상 소요 시간": mins,
        "직항여부": directInt,
        "직항/경유": directTxt,
                  ...
                  children: [
                      ...
                      child: Autocomplete<String>(
                        ...
                        fieldViewBuilder:
                          ...
                          if (textController.text.isEmpty &&
                              _origin.text.isNotEmpty) {
                            textController.text = _origin.text;
                    ...
                    Expanded(
                      child: TextFormField(
                        ...
                        enabled: false,
                        decoration:
                            const InputDecoration(labelText: '목적지 (ICN)'),
                      ),
                    ),
                  ],
                ),
